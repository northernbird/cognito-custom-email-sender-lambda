...
locals {
  update_user_pool_command = "aws cognito-idp update-user-pool --user-pool-id ${local.cognito_id} --lambda-config \"CustomSMSSender={LambdaVersion=V1_0,LambdaArn=${local.custom_sms_sender_lambda}},KMSKeyID=${local.kms_key}\""
}
