...
locals {
  custom_sms_sender_lambda = "arn:aws:lambda:ap-northeast-1:420903242724:function:mynt-jp-sms-sender-lambda-s-myntCustomSMSSenderSam-RIWc0YeBYhdQ"
  kms_key  = "arn:aws:kms:ap-northeast-1:420903242724:key/f0b22564-1667-43d3-a9d9-c87fb713591c"
}

locals {
  update_user_pool_command = "aws cognito-idp update-user-pool --user-pool-id ${aws_cognito_user_pool.cognito_user_pool.id} --cli-input-json file://${var.update_user_pool_config_file} --lambda-config \"CustomEmailSender={LambdaVersion=V1_0,LambdaArn=${local.custom_sms_sender_lambda}},KMSKeyID=${local.kms_key}\""
}
